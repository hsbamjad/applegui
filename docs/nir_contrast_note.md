...
| Per-channel `NegotiatePacketSize(channel_index)` | AVOID: Overwrites `SetStreamDestination` -> TIMEOUT on all channels |
...
- AVOID: `device.NegotiatePacketSize(channel_index)` inside `open()` - **breaks stream routing**
- AVOID: Sequential (one-source-at-a-time) acquisition for the production camera worker - loses hardware sync
- AVOID: Trying to boost NIR values through software gain - adds noise, doesn't recover actual scene data
