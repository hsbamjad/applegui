...
```
Side view of screw conveyor (axis runs left-right, screw rotates in place):
...
(O) = apple sitting in screw cup
One full rotation = apple advances exactly one pitch toward exit
```
...
frames_in_view       = transit_time x inference_fps
```
...
|---|---|---|
| `min_frames = 5` satisfied? | Risk at high speed | Easy at all speeds |
...
frames_in_view = 2.0 s x 15 FPS        = 30 frames

30 frames >> min_frames=5  - confirmed safe at all speeds
```

Also confirmed from the image: conveyor orientation is **LR** (left to right),
consistent with `config.yaml -> conveyor.orientation: "LR"` for simulation.

- Counted visible apples in frame: **6 pitches per lane**
- Verified: `6 pitches / 3 apples/s x 15 FPS = 30 frames >= 5` - confirmed
